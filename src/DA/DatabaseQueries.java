...
public class DatabaseQueries {
    ...
    public  static List<QueryRecord> importData() throws SQLException {
        ...
        String sql="SELECT S.ZAttribut04 as Kappstelle, ST.TATTRIBUT7 as Block, S.ArtikelNr, ART.ARTIKELBEZEICHNUNG, CONVERT(numeric(15, 2), AP.Fertdauer / 60 / 60) AS FertdauerPVM, Ap.FERTIGTERMINDATUM, S.AuftragsNr, S.AuftragsPos, A.Farbe1\n" +
                "FROM FLS_SCHRITTE S\n" +
        ...
        while (rs.next()) {
            ...
            String color = rs.getString("Farbe1");

            QueryRecord record = new QueryRecord(workplaceID, block, itemNumber, itemName, productionTime, productionDate, orderNumber, orderPos,color);
            records.add(record);
        }
        conn.close();
        return records;
